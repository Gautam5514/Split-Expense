  ...
  return (
    <section className="relative w-full min-h-screen overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 pt-20 sm:pt-28 flex flex-col items-center">
        ...
        >
          {/* Browser chrome — hidden on very small screens, shown sm+ */}
          <div className="hidden sm:block">
            ...
            >
              <div className="flex items-center gap-4 px-4 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                ...
                <div className="flex gap-1.5">
                  {[Bell, Settings].map((Icon, i) => (
                    <div key={i} className="w-6 h-6 rounded-md flex items-center justify-center" style={{ background: "rgba(255,255,255,0.05)" }}>
          ...
          {/* Dashboard UI */}
          <div
            className="flex overflow-hidden rounded-2xl sm:rounded-t-none sm:rounded-b-2xl"
            style={{
          ...
          >
            {/* Sidebar — tablet+ */}
            <div
              className="hidden md:flex w-44 lg:w-52 flex-shrink-0 flex-col py-5 px-3"
              style={{ borderRight: "1px solid rgba(255,255,255,0.06)" }}
            >
              <div className="flex items-center gap-2 px-2 mb-7">
                <div className="w-7 h-7 rounded-lg overflow-hidden flex items-center justify-center border border-white/10 flex-shrink-0">
              ...
              <div className="flex flex-col gap-0.5">
                {NAV.map(({ icon: Icon, label, active, badge, sparkle }) => (
                  <div
                    ...
                    style={active ? { background: "linear-gradient(135deg,rgba(8,145,178,0.28),rgba(6,182,212,0.18))", border: "1px solid rgba(8,145,178,0.25)" } : {}}
                  >
                    <div className={`flex items-center gap-2.5 text-xs font-medium ${active ? "text-white" : sparkle ? "text-cyan-400" : "text-white/40"}`}>
                      {sparkle ? <Sparkles className="w-3.5 h-3.5" /> : <Icon className="w-3.5 h-3.5" />}
                      {label}
                    </div>
                    {badge && (
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: "rgba(8,145,178,0.3)", color: "#67E8F9" }}>
                        {badge}
                      </span>
                    )}
                  </div>
              ...
              <div className="mt-auto flex items-center gap-2 px-2 py-2.5 rounded-xl" style={{ background: "rgba(255,255,255,0.04)" }}>
                <img src="https://api.dicebear.com/7.x/notionists/svg?seed=Felix&backgroundColor=c4b5fd" className="w-7 h-7 rounded-lg object-cover bg-teal-900" alt="user" />
            ...
            <div className="flex-1 flex flex-col min-w-0">
              ...
              >
                <div>
                  ...
                  <p className="text-[9px] sm:text-[10px] text-white/30 mt-0.5">4 pending expenses</p>
                </div>
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    className="flex items-center gap-1 px-2 py-1.5 sm:px-3 rounded-xl text-[10px] sm:text-xs text-white font-bold whitespace-nowrap"
              ...
              <div className="flex flex-1 overflow-hidden">
                <div className="flex-1 p-3 sm:p-4 flex flex-col gap-3 overflow-y-auto min-w-0 scrollbar-none">

                  {/* Balance cards */}
                  <div className="grid grid-cols-3 gap-1.5 sm:gap-2.5 flex-shrink-0">
                    {[
                      { label: "Expenses", value: "₹60,600", Icon: Receipt, color: "text-white" },
                      { label: "You Owe", value: "₹3,240", Icon: TrendingDown, color: "text-red-400", bg: "rgba(239,68,68,0.08)", border: "rgba(239,68,68,0.2)" },
                      { label: "Owed", value: "₹7,800", Icon: TrendingUp, color: "text-emerald-400", bg: "rgba(52,211,153,0.08)", border: "rgba(52,211,153,0.2)" },
                    ].map(({ label, value, Icon, color, bg, border }) => (
                      <div
                        key={label}
                        className="p-2 sm:p-3 rounded-xl"
                        style={{ background: bg || "rgba(255,255,255,0.04)", border: `1px solid ${border || "rgba(255,255,255,0.08)"}` }}
                      >
                        <div className="flex items-center justify-between mb-1">
                          <p className="text-[8px] sm:text-[9px] text-white/40 uppercase tracking-wider font-semibold truncate">{label}</p>
                          <Icon className={`w-3 h-3 ${color} flex-shrink-0`} />
                        </div>
                        <p className={`text-[10px] sm:text-sm font-extrabold ${color}`}>{value}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex-shrink-0">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-[10px] sm:text-xs font-bold text-white">Your Groups</h3>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 sm:gap-2">
                      {GROUPS.map(({ name, emoji, members, total, color, pct }) => (
                        <div
                          key={name}
                          className="p-2 sm:p-2.5 rounded-xl cursor-pointer"
                          style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}
                        >
                          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl mb-1.5 flex items-center justify-center text-sm" style={{ background: `${color}22`, border: `1px solid ${color}40` }}>
                            {emoji}
                          </div>
                          <p className="text-[9px] sm:text-[10px] font-bold text-white leading-tight mb-0.5 truncate">{name}</p>
                          <p className="text-[8px] sm:text-[9px] text-white/30 mb-1.5 truncate">{members}m · {total}</p>
                          <div className="w-full h-1 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
                            <div className="h-full rounded-full" style={{ width: `${pct}%`, background: `linear-gradient(90deg, ${color}, ${color}99)` }} />
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1.5">
                      <h3 className="text-[10px] sm:text-xs font-bold text-white">Recent Expenses</h3>
                      <span className="text-[9px] text-cyan-400 cursor-pointer font-semibold">View all →</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      {EXPENSES.map(({ name, person, amount, time, emoji, settled }) => (
                        <div
                          key={name}
                          className="flex items-center justify-between px-2 sm:px-2.5 py-1.5 rounded-lg sm:rounded-xl cursor-pointer"
                          style={{ border: "1px solid rgba(255,255,255,0.05)" }}
                        >
                          <div className="flex items-center gap-2 min-w-0">
                            <span className="text-sm flex-shrink-0">{emoji}</span>
                            <div className="min-w-0">
                              <p className="text-[9px] sm:text-[10px] font-semibold text-white truncate">{name}</p>
                              <p className="text-[8px] sm:text-[9px] text-white/30 truncate hidden sm:block">by {person} · {time}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 flex-shrink-0">
                            <p className={`text-[9px] sm:text-[10px] font-bold ${settled ? "text-emerald-400" : "text-white"}`}>{amount}</p>
                            <span className={`text-[7px] sm:text-[8px] font-bold px-1.5 py-0.5 rounded-full hidden sm:block ${settled ? "text-emerald-400 bg-emerald-400/10" : "text-amber-400 bg-amber-400/10"}`}>
                ...
                <div
                  className="hidden lg:flex w-48 xl:w-52 flex-shrink-0 flex-col p-3.5 gap-3.5 overflow-hidden"
                  style={{ borderLeft: "1px solid rgba(255,255,255,0.05)" }}
                >
                  {/* AI insight */}
                  <div className="p-3 rounded-2xl" style={{ background: "linear-gradient(135deg,rgba(8,145,178,0.2),rgba(6,182,212,0.1))", border: "1px solid rgba(8,145,178,0.28)" }}>
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-cyan-400" />
                      <span className="text-[10px] font-bold text-cyan-300">AI Insight</span>
                    </div>
                    <p className="text-[10px] text-white/60 leading-relaxed">
                      You spend <strong className="text-white/90">₹2,400 more</strong> on dining while traveling.
                    </p>
                    <button className="mt-1.5 flex items-center gap-1 text-[9px] font-bold text-cyan-400">
                  ...
                  {/* Settle up */}
                  <div>
                    <p className="text-[9px] font-bold text-white/40 uppercase tracking-widest mb-1.5">Settle Up</p>
                    <div className="flex flex-col gap-1.5">
                      {[{ name: "Priya", amount: "₹1,200", seed: "Priya" }, { name: "Alex", amount: "₹2,040", seed: "Alex" }].map(({ name, amount, seed }) => (
                        <div key={name} className="flex items-center justify-between p-2 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                          <div className="flex items-center gap-1.5">
                            <img src={`https://api.dicebear.com/7.x/notionists/svg?seed=${seed}&backgroundColor=c4b5fd`} className="w-6 h-6 rounded-lg bg-teal-900" alt={name} />
                            <div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-[9px] text-cyan-400 font-semibold flex items-center gap-0.5"><TrendingUp className="w-2.5 h-2.5" /> +12%</span>
                    </div>
                    <div className="relative h-14">
                      <svg viewBox="0 0 150 55" className="w-full h-full" preserveAspectRatio="none">
                        ...
                        <path d="M0,50 C18,44 28,30 45,25 C62,20 72,36 88,22 C104,8 118,26 135,10 L150,5" fill="none" stroke="#22D3EE" strokeWidth="2" strokeLinecap="round" />
                        <path d="M0,50 C18,44 28,30 45,25 C62,20 72,36 88,22 C104,8 118,26 135,10 L150,5 L150,55 L0,55 Z" fill="url(#chartFill)" />
                      </svg>
                      <div className="absolute bottom-0 w-full flex justify-between px-0.5">
                        {["J","F","M","A","M","J"].map((m, i) => <span key={i} className="text-[8px] text-white/20">{m}</span>)}
                      </div>
                    </div>
                  </div>
                </div>
