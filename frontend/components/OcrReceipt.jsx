...
const Section = ({ title, children, className = "" }) => (
  <div className={`mb-6 ${className}`}>
    {title && (
      <h3 className="text-primary text-sm font-semibold mb-3 uppercase tracking-wider border-b border-border pb-2">
        {title}
      </h3>
    )}
    <div className="text-muted-foreground leading-relaxed text-sm font-mono">
      {children}
    </div>
  </div>
);
...
export default function OcrReceipt({ text }) {
  ...
  if (!parsedData) {
    return <p className="text-muted-foreground font-mono">No parsable OCR text available.</p>;
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
          <h3 className="font-bold text-lg text-foreground mb-2">LOGO TEXT</h3>
        ...
        <div className="text-left md:text-right">
          <h2 className="font-bold text-2xl text-foreground uppercase tracking-wider mb-2">Tax Invoice</h2>
      ...
      {buyer.name.length > 0 && (
        <Section title="Billed To">
          <p className="font-semibold text-foreground">{buyer.name.join(' ')}</p>
